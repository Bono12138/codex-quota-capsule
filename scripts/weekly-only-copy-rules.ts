...
const ambiguousResetCopy = /周额度刷新|本周额度[^\n]{0,40}刷新(?:前|后)|可能在刷新前用完|撑不到刷新/;

export function ambiguousResetCopyReason(line: string): string | null {
  return ambiguousResetCopy.test(line) ? "quota reset is mislabeled as refresh" : null;
}
