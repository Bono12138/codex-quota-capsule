...
- 胶囊和展开面板均以双条对比为主：时间条同时标出自然时间与可用时间，下面是周额度已用进度。两条总宽度一致。
- 常驻胶囊默认 180×60 点，直接显示自然时间、可用时间和额度已用百分比；110×32 点的贴边胶囊用沙漏、闪电配合双条与数值速览。悬停查看截止时间，点击展开详情。
- 默认时段外，可用时间进度暂停，自然时间继续走。两种时间共享周期起止点，可以交叉；这代表计划可用时间，不是监测到的在线时长。
- 提示语按额度状态变化，同一天、同一状态保持稳定。5 小时限制单独显示，预算细节和历史趋势按需展开。
