...
这个仓库作为 public 发布仓库和 public issues 入口。private 工作仓库继续保留内部交接、未公开规划和发布准备。
...
第一分发方式是 GitHub + Codex-assisted 安装提示词。
...
脚本存在后，也要先看脚本输出的审查结果，再把 staging directory 推到 public 仓库。
