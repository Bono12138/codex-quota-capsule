...
用户看到社交媒体介绍后，不是先下载一个陌生 zip，而是把提示词交给自己的 Codex，让 Codex 在本机完成 clone、检查环境、构建和启动。
...
它的价值不是宣传、分发流量或支付系统，而是 Mac app 分发的信任基础设施：
...
所以，99 美元不是“买渠道”，而是“买 Mac 认可的开发者身份和分发信任”。
...
关键不只是有没有工商执照，而是 Apple 和 D&B 是否能把该主体识别为可用于 Apple Developer Program 的 legal entity，并生成或匹配 D-U-N-S Number。
...
1. 右键点击 Quota Capsule.app，选择“打开”。
...
Quota Capsule 的用户不是普通大众，但也不一定是本地开发环境专家。
...
> 早期先把 Codex 用户的安装路径做顺，而不是先把传统 Mac app 分发流程做重。
