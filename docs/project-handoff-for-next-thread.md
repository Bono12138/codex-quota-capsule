...
它把 Codex 的 quota window 翻译成用户真正关心的问题：
...
当前截图更符合 Mac 环境找不到 Codex CLI 的诊断，需要先排除额度用完。
...
远程产品分析必须有三语 consent、设置开关、采集边界和事件 schema。问题上报、更新提醒仍需单独设计，不能默认静默上传。
...
3. 建立简中、繁中、英文三语文案体系、首屏语言判断和缺失 key 检查。
4. 重做分步新手引导、失败诊断引导、胶囊和详情面板信息架构。
5. 做本地 SQLite 历史库、事件指标、远程产品分析 consent、事件队列和上报 adapter。
6. 完成联系作者 / 反馈入口、抖音二维码可扫性和复制反馈。
7. 准备 public 仓库文件清单和 Codex-assisted 分发文案。
8. 后续再考虑 Spec Kit 或更完整的验收宪法。

推荐优先级：

```text
P0：极端状态测试 + Codex CLI 查找诊断
P1：三语界面、首屏语言策略、缺失 key 检查
P1：分步新手引导、失败诊断引导、胶囊和详情面板重排
P1：本地 SQLite 历史库、事件指标、远程产品分析 consent、事件队列和上报 adapter
P1：联系作者 / 反馈入口、抖音二维码和复制反馈
P1：刷新图标语义修正
P2：半隐藏贴边形态
P2：幽默文案体系
P3：语音、更新提醒
...
- 远程产品分析是下一版正式需求，用来理解日活、留存、常驻胶囊时长、引导转化、读取失败、反馈入口点击和版本质量；必须有三语 consent、设置开关和采集边界。
- 本地历史数据和远程产品分析分开实现：本地 SQLite 记录 quota 快照和本机事件，远程侧只上传经同意的产品使用事件、粗粒度环境信息和错误类型。
...
- 不要把环形图、阈值设置、语音、更新提醒直接当成默认要做。
- 远程产品分析是 P1，但必须按 consent、事件 schema 和采集边界实现。
