...
- 菜单栏常驻项使用极简图标，减少被 macOS 菜单栏拥挤状态挤掉的概率；状态和 5 小时窗口已用比例放在 tooltip、菜单头部和桌面胶囊里。
- 菜单栏常驻项不可作为唯一入口。刷新、反馈、设置、作者信息和关于页必须同时能从桌面胶囊展开面板进入。
...
- 状态栏必须由 AppKit `NSStatusItem` 持有，使用极简图标常驻；状态 + 5 小时已用百分比放在菜单头部和 tooltip，避免长文案被 macOS 挤出菜单栏。
- 状态栏菜单必须有一级“联系作者”入口，点击一次即可看到作者、邮箱、X、抖音号和常用操作。
- 展开面板必须在详情顶部提供浅层入口：刷新、提交反馈、作者 X 主页、关于与反馈、高级数据设置。状态栏不可见或被系统折叠时，用户仍能找到反馈和设置。
