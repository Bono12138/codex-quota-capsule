...
- 通用 Codex 桶返回有效 300 分钟窗口时，收起胶囊和详情面板必须显示真实 5 小时已用进度；详情同时显示剩余比例和精确到分钟的重置时间。
- 通用 Codex 桶没有返回 5 小时窗口时，不显示假进度；详情明确说明 Codex 暂未提供该读数。
- 5 小时窗口时长只接受 300 分钟附近的有限容差，重置必须在未来约 6 小时内；15 分钟、1 天、过期或异常窗口不得重命名为 5 小时额度。
- 通用 Codex 额度桶优先于兼容字段；Spark 等模型独立额度不得冒充通用 Codex 额度。
- 5 小时读数不进入周速度、未来 24 小时预算或周重置余量计算。
