...
- 当前周期所有已接受读数仍为 0% 时，若 `resetsAt` 的移动量与两次读取间隔在五分钟容差内一致，必须把它识别为尚未使用窗口的临时锚点并跟随最新读数；第一次正向用量出现后立即停止跟随，不能继续冻结在旧的 0% 和旧重置时间。
...
- 共享 fixture 必须覆盖“多个滑动的 0% 临时重置时间 → 固定重置时间上的正向用量”序列，并断言双端采用最新百分比而不是停留在校准态。
