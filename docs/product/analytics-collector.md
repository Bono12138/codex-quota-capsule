...
`quota_state_sampled` 会发送周窗口用量、当前刷新进度、按下一刷新点计算的可持续日速度、最近速度区间、预计刷新余量区间、预测状态与置信度，以及胶囊宽度分档和展开状态。它不包含重置券数量、到期时间或指纹，也不包含 prompt、session、token、文件路径、项目名、窗口标题、代码或命令。
