Status: Historical feature inventory. Superseded by ../product/feature-roadmap.md on 2026-09-20.
...
> 常驻小胶囊，扫一眼知道本周额度按当前节奏能否撑到重置，以及今天还能用多少。
...
- 初步估算 / 够用 / 波动较大 / 可能不够 / 已用尽 / 数据暂不可用。
- 能不能撑到周额度重置。
- 如果撑不到，预计几点见底。
- 如果能撑到，重置时预计剩多少。
- 未来 24 小时按可持续速度建议不超过多少。

计算依据：

- 时间进度。
- 额度已用。
- 周期、最近、活动节奏与弱历史先验形成的有界速度证据。
- 剩余时间。
- 剩余额度。
- 证据可靠度、真实增长次数和估算分歧。

产品价值必须从第一次有效读数开始。后续历史用于收窄区间和提高置信度，不设置固定等待门槛。
...
- Backlog：账户级 token 趋势、峰值日、本周 token 节奏和周报建议。
- Later：高级本地会话分析，需要单独授权，默认不读取原始 session 文件。
- Later：跨 provider 比较和计划推荐，优先使用官方 usage/cost API、用户导入和公开计划数据。
...
- 在展开面板最下方逐张显示本地到期时间，精确到分钟。
- 本地长期保留发放、到期、首次/最后看到时间和生命周期状态。
- 原始券 ID 不落盘、不上传；只保存单向指纹用于去重。
- 不把券数量或即将到期误写成“额度安全”。
- 把最早可用券到期与自然周重置取较早者，作为当前预算的下一刷新点；刷新后重新读取新周期和剩余券。

当前默认策略明确假设用户会在最早券到期前使用它，但不自动兑换。后续独立能力才是复杂的重置券使用策略：它需要消费多个未来需求情景、券到期、兑换后周期变化、任务价值和风险偏好，并通过滚动动态规划给出可解释建议。

研究与数学边界见 [`docs/research/reset-credit-timing-optimization.md`](../research/reset-credit-timing-optimization.md)。在历史回测证明优于“接近耗尽使用”和“到期前使用”等简单基线前，不进入默认 UI，也不自动兑换。
...
每次采样记录一个 capture，并为 weekly 窗口写入一条原始 window record。所有速度、余量区间和风险结论在读取时重新计算，不把旧算法的派生值当作事实保存。
...
- `window_type`：weekly / other（为未来 provider 保留扩展位）
- `window_minutes`
- `used_percent`
- `remaining_percent`
- `resets_at`
...
原始本地快照不持久化单一 burn-rate 或预测结论。`earlyEstimate / enough / watch / mayRunOut / exhausted / unavailable`、证据区间、置信原因和未来 24 小时建议由当前算法从清洗后的历史重新计算，避免旧派生字段在算法升级后伪装成最新结论。
...
- 每分钟刷新一次，每次最多写一条 weekly 原始记录，每天约 1440 条；预测器按五分钟桶和用量变化做有效采样。
- 30 天约 43200 条原始 window record。
...
- `provider`：codex
- `status`：earlyEstimate / enough / watch / mayRunOut / exhausted / unavailable
...
- `width_bucket`
- `language`
- `feedback_target`：email / github / x / douyin_open / douyin_copy / codex_prompt
