...
额度胶囊同时处理两个不同的问题：5 小时额度告诉你眼前还能不能继续用；周额度预测告诉你照现在的节奏能不能撑到下一次刷新。下一刷新点取自然周重置与最早可用重置券到期时间中更早者，并据此显示未来 24 小时建议和刷新时的预计余量区间。

只有 Codex 返回真实的 300 分钟通用额度窗口时，应用才显示 5 小时进度和精确到分钟的重置时间。接口暂未返回该窗口时，应用会如实说明读数尚未提供，不会用零进度或其他模型的独立额度代替。
...
- 桌面悬浮胶囊显示周判断、周已用比例，以及可用时的 5 小时进度。
- 菜单栏提供随时可见的一眼状态。
...
- 按真实窗口时长识别通用 Codex 的 5 小时额度；不跨模型额度桶拼接数据。
